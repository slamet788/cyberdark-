...
def tampilhasil(akun,sandi,data):
	...
	i = inputD('[?] Jika ingin Hack lagi ,Pilih y untuk mulai (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] SUBSCRIBE DULU CHANEL Rafly pake i...')
		log = 1
